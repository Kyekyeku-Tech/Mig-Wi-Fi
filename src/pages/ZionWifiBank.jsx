    ...
    <div className="overflow-hidden relative w-full h-full flex justify-center items-center">
      <AnimatePresence initial={false} mode="wait">
        ...
        >
          ...
          <a
            href={`https://wa.me/233243767677?text=${encodeURIComponent(
              whatsappPrefix(items[index])
            )}`}
            target="_blank"
...
export default function ZionWifiBank() {
    ...
    async function loadPackages() {
      try {
        ...
        const pkgs = docs.length
          ? docs
          : [
              { id: "Immigration-32d", name: "Immigraton — 1 Month", price: 100, description: "32 days 3 device Unlimited" },
              { id: "Custom-32d", name: "Custom — 1 Month", price: 100, description: "32 days 3 device Unlimited" },
              { id: "Police-32d", name: "Police —  1 Month", price: 100, description: "32 days 3 device Unlimited" },
            ];
        setPackages(pkgs);
        ...
        for (let pkg of pkgs) {
          ...
          avail[pkg.id] = snapCreds.docs.length >= 3; // must have 3 available
        }
  ...
  const generateTicket = ({ reference, pkg, creds }) => {
    ...
    let y = 80;
    creds.forEach((c, i) => {
      docPDF.text(`Device ${i + 1} Username: ${c.username}`, 20, y);
      y += 10;
      docPDF.text(`Device ${i + 1} Password: ${c.password}`, 20, y);
      y += 10;
    });

    docPDF.text(`Reference: ${reference}`, 20, y + 10);
    docPDF.text(`Date: ${new Date().toLocaleString()}`, 20, y + 20);
    ...
    let y2 = 240;
    creds.forEach((c, i) => {
      ctx.fillText(`Device ${i + 1} Username: ${c.username}`, 50, y2);
      y2 += 30;
      ctx.fillText(`Device ${i + 1} Password: ${c.password}`, 50, y2);
      y2 += 40;
    });

    ctx.fillText(`Reference: ${reference}`, 50, y2);
  ...
  const payPackage = async (pkg) => {
    if (!phone || !name)
      return alert("Enter your name and phone number.");
    if (!availability[pkg.id])
      return alert(`No credentials available for ${pkg.name}.`);
...
try {
  ...
  const handlePaymentSuccess = async (reference, pkg) => {
    ...
    try {
      ...
      if (snap.empty || snap.docs.length < 3) {
        alert("Payment succeeded but not enough credentials available (need 3).");
        return;
      }

      const selectedCreds = snap.docs.slice(0, 3).map((d) => ({
        id: d.id,
        ...d.data(),
      }));

      // Mark all 3 as used
      for (let cred of selectedCreds) {
...
${selectedCreds
  .map(
    (c, i) => `Device ${i + 1}: ${c.username} / ${c.password}`
  )
  .join("\n")}
...
Thank you for choosing Starlink WiFi Bank.`;
  ...
  const cardClass = (isAvailable) =>
    theme === "dark"
      ? `p-6 rounded-2xl shadow transition transform ${
          isAvailable
            ? "bg-white/10 hover:-translate-y-2"
            : "bg-white/20 opacity-50 cursor-not-allowed"
        }`
      : `p-6 rounded-2xl shadow transition transform ${
          isAvailable
            ? "bg-gray-100 hover:-translate-y-2"
            : "bg-gray-200 opacity-50 cursor-not-allowed"
        }`;

  const inputClass =
    "p-2 rounded placeholder:text-slate-400 " +
    (theme === "dark"
      ...
      <div className="max-w-5xl mx-auto">
        <header className="mb-8 flex flex-col md:flex-row justify-between items-center gap-4">
          <div>
            <h1
              className={
                theme === "dark"
                  ? "text-3xl font-bold text-sky-400 mb-2"
                  : "text-3xl font-bold text-blue-600 mb-2"
              }
            >
              Starlink WiFi Bank
            </h1>
          ...
          <div className="flex gap-3">
            ...
            <a
              href="/admin/login"
              className="bg-red-600 text-white px-4 py-2 rounded-full font-bold hover:bg-red-500"
            >
              Admin
            </a>
          </div>
              ...
              return (
                <div key={p.id} className={cardClass(isAvailable)}>
                  <h3
                    className={
                      theme === "dark"
                        ? "text-xl font-bold text-sky-400 mb-2"
                        : "text-xl font-bold text-blue-600 mb-2"
                    }
                  >
                    {p.name}
                  </h3>
                  ...
                  <div className="flex items-center justify-between">
                    <div
                      className={
                        theme === "dark"
                          ? "text-2xl font-extrabold text-sky-100"
        ...
        <section className="mt-6 relative h-[300px] md:h-[250px]">
          ...
          <AutoCarousel
            ...
            theme={theme}
            whatsappPrefix={(item) =>
              `Hello Admin, I want the ${item.title}.`
            }
          />
        </section>
        ...
        <section className="mt-6 relative h-[300px] md:h-[250px]">
          ...
          <AutoCarousel
            ...
            theme={theme}
            whatsappPrefix={(item) =>
              `Hello Admin, I want to order the ${item.speed} Dedicated Internet Package.`
            }
          />
        </section>
      </div>
